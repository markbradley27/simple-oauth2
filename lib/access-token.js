...
module.exports = class AccessToken {
  ...
  */
  async refresh(params = {}, httpOptions = {}) {
    ...
    const parameters = GrantTypeParams.forGrantType(REFRESH_TOKEN_PROPERTY_NAME, this.#config, refreshParams);
  ...
  * @returns {Promise}
  */
  async revoke(options, httpOptions = {}) {
    Hoek.assert(
      options.tokenType === ACCESS_TOKEN_PROPERTY_NAME || options.tokenType === REFRESH_TOKEN_PROPERTY_NAME,
    ...
    const tokenParamName = options.tokenParamName ?? 'token';
    requestOptions[tokenParamName] = this.token[options.tokenType];

    return this.#client.request(this.#config.auth.revokePath, requestOptions, httpOptions);
  }
